function [success] = high_vol_test(mpc)
[YBUS, ~, ~] = makeYbus(mpc);
[x,y,value] = find(YBUS);
...
for loop = 1 : length(mpc.bus(:,1))
    ...
    if mpc.bus(loop,2) == 1
        ...
        Q_load(count_load_index) = mpc.bus(loop,4) / mpc.baseMVA;
        %bus(:,6) is B_s(reactive part for I_shunt)
        I_shunt(count_load_index) = mpc.bus(loop,6) / mpc.baseMVA;
        %I_shunt(count_load_index) = 0;
        %V_load_star(count_load_index) = bus(loop,8);
    end
    %if generator bus
    %further in gen part
    if (mpc.bus(loop,2) == 2) || (mpc.bus(loop,2) == 3)
...
for i = 1 : length(mpc.gen(:,1))
    [li_gen,loc_gen] = ismember(mpc.gen(i,1),Generator_index);
    if li_gen == 1 
        V_G(loc_gen) = mpc.gen(i,6);
    end
end
